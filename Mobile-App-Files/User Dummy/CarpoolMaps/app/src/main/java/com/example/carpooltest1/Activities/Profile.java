...
public class Profile extends AppCompatActivity {
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            //setting up profile
                            txt_name.setText(jsonObject.getString("Name") + "(" +  SharedPrefManager.getInstance(getApplicationContext()).getUsername() + ")");
    ...
    private void ShowList() {
        ...
        Log.d(TAG, "ShowList: " + SharedPrefManager.getInstance(this).getUsername());
        StringRequest GetReviews_requets = new StringRequest(Request.Method.POST, URL_GETREVIEWS,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        progressDialog.dismiss();
                        Log.d(TAG, "Profile onResponse: Response Recieved");
                        Reviews UserR;
                        try {
                            JSONArray jsonArray = new JSONArray(response);
                            //Load wala kaam
                            Log.d(TAG, "onResponse: " + response);
                            boolean NO_REVIEWS = true;
                            for(int i= 0;i < jsonArray.length();i++){
                                JSONObject jsonObject = jsonArray.getJSONObject(i);
                                if((jsonObject.getInt("Status") > 7
                                                &&
                                                jsonObject.getInt("Status") <  11)
                                                ||
                                                (jsonObject.getInt("Status") > 13
                                                        &&
                                                        jsonObject.getInt("Status") < 17)){
                                    NO_REVIEWS = false;
                                    UserR = new Reviews(jsonObject.getString("Reviews_D"),
                                            jsonObject.getInt("Rating_D"),
                                            jsonObject.getString("DriverID"));
                                    Reviewlist.add(UserR);
                                }
                                else{
                                    Log.d(TAG, "onResponse: Review not found at" + i);
                                }
                            }
                            if(NO_REVIEWS){
                                UserR = new Reviews("No Reviews Available",1,"");
                                Reviewlist.add(UserR);
                            }
        ...
        RequestHandler.getInstance(this).addToRequestQueue(GetReviews_requets);
